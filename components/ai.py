...
class BaseAI(Action):
    ...
    def get_path_to(self, dest_x: int, dest_y: int) -> List[Tuple[int, int]]:
        ...
        cost = np.array(self.entity.gamemap.tiles[TA_WALKABLE], dtype=np.int8)

        for entity in self.entity.gamemap.entities:
            # Check that an enitiy blocks movement and the cost isn't zero
            # (blocking.)
...
class ConfusedEnemy(BaseAI):
    ...
    def perform(self) -> None:
        # Revert the AI back to the original state if the effect has run its
        # course.
        if self.turns_remaining <= 0:
            ...
            # direction. Its possible the actor will just bump into the wall,
            # wasting a turn.
